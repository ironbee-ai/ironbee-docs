...
An **activity** is a discrete working period within a session. Activities form the chronological record of everything the agent did — each one contains a mix of verifications, fixes, and other tool calls.
...
| `other` | Tool calls and events that don't belong to a verification or fix — idle time, setup, general tool use |
...
Shows when you click an "other" interval — events that occurred outside a named verification or fix:
