...
IronBee supports two cycle types that can run in parallel:
...
The browser cycle is active by default for most code file types.

### Node cycle

The agent connects to a running Node.js process to verify backend changes:
...
The Node cycle requires opt-in — see [`ironbee enable-backend node`](/cli/setup#backend).
