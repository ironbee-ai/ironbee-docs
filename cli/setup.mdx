---
title: Project Setup
description: Configure IronBee hooks, skills, and MCP servers in your project.
---

## Before you start

Make sure you have logged in first:

```bash
ironbee login
```

This connects the CLI to your IronBee account. If you haven't done this yet, see the [Quick Start](/getting-started/quickstart#step-3--connect-the-cli-to-your-account).

---

## Run the installer

From your project root:

```bash
ironbee install
```

IronBee auto-detects your AI client. To specify it explicitly:

```bash
ironbee install --client claude   # Claude Code
ironbee install --client cursor   # Cursor
```

### What gets installed

| File / Setting | Description |
|----------------|-------------|
| Hook configuration | Tells the client to call IronBee when the agent finishes a task |
| Verification skill | Instructions the agent follows during verification |
| Verification rule | Enforces the verification gate |
| `browser-devtools` MCP server | Browser mode — `bdt_` prefix tools for navigation, screenshots, console. Always registered. |
| `node-devtools` MCP server | Node mode — `ndt_` prefix tools. Registered only after `ironbee node enable`. |
| `backend-devtools` MCP server | Backend protocol mode — `bedt_` prefix tools for HTTP/gRPC/GraphQL/WebSocket. Registered only after `ironbee backend enable`. |
| Permissions | Grants the agent access to all registered devtools tools |

---

## Cursor: additional activation

After running `ironbee install` in a Cursor project:

1. **Restart Cursor** to load the new hooks and MCP config
2. Go to **Settings → Tools & MCP** and confirm the registered devtools servers are listed as enabled. Only cycles you have enabled appear: `browser-devtools` is registered by default; `node-devtools` and `backend-devtools` are added when you run `ironbee node enable` or `ironbee backend enable`
3. If a server shows as enabled but tools are unavailable, toggle it off and back on

<Warning>
  This is a [known Cursor limitation](https://forum.cursor.com/t/mcp-tools-only-available-to-agent-after-manually-toggling-server-off-on-even-when-already-enabled/152859) — MCP servers added via `mcp.json` sometimes need manual activation.
</Warning>

---

## Enable Node.js runtime verification {#backend}

By default, only the browser cycle is active. To also require Node.js V8 inspector verification for backend file changes:

```bash
ironbee node enable
```

This writes a minimal `{ "node": {} }` block to your project config. Code defaults (e.g. `server/**`, `pages/api/**`, `**/server.{ts,js,mjs,cjs}`) apply at runtime — nothing is materialized into the file.

To disable:

```bash
ironbee node disable
```

## Enable backend protocol verification

For any backend runtime (Node, Java, Python, Go, Rust, and more), you can require the agent to exercise real HTTP, gRPC, GraphQL, or WebSocket calls before completing:

```bash
ironbee backend enable
```

This writes a minimal `{ "backend": {} }` block to your project config. Code defaults covering `server/**`, `api/**`, `routes/**`, `controllers/**`, `handlers/**`, and `services/**` apply at runtime.

To disable:

```bash
ironbee backend disable
```

## Disable the browser cycle

For backend-only projects where browser verification is not appropriate:

```bash
ironbee browser disable
```

To re-enable:

```bash
ironbee browser enable
```

---

## Monitoring-only mode (no enforcement)

If you want to collect session data without requiring the agent to verify:

```bash
ironbee verification disable
```

In this mode:
- The enforcement hook, verification skill, and MCP servers are **not installed**
- Session lifecycle events, tool calls, and timing still flow to the IronBee Collector
- The agent never sees a verify gate

To re-enable enforcement:

```bash
ironbee verification enable
```

<Note>
  Restart your editor or agent session after toggling — the change takes effect on the next session.
</Note>

---

## Check status

To see the verdict status of active sessions in a project:

```bash
ironbee status
```

---

## Remove IronBee from a project

```bash
ironbee uninstall
```

This removes all hooks, skills, rules, and MCP server entries IronBee added.

---

## Next step

<Card title="Configuration" icon="settings" href="/cli/configuration">
  Customize verify patterns, retry limits, and devtools settings.
</Card>
