...
| `COLLECTOR_ARTIFACTS_ENABLE=false` | Drops **screenshots and recordings** (browser artifacts). |
...
Privacy mode flips both flags together. If you want, say, recordings off but tool detail on, set the per-cycle [DevTools env override](/cli/configuration/configuration#devtools-mcp-overrides) — your `*DevTools.env` values are applied *after* the privacy flags, so they win:
...
Here screenshots and recordings stay redacted (privacy mode), but browser tool detail is allowed back through.
