...
<Info>
  The legacy per-cycle keys (`browserDevTools`, `nodeDevTools`, `backendDevTools`, `androidDevTools`, `terminalDevTools`) still exist for back-compat but **no longer feed the compose server** — use `ironbeeDevTools` to customize the one server that serves every cycle.
</Info>
