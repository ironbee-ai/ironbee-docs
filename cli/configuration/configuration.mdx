...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `verification.enable` | `boolean` | `true` | Master enforcement switch. `false` = monitoring-only (telemetry still flows, no verify gate). Equivalent to [`ironbee verification disable`](/cli/guides/verification#enforcement-vs-monitoring-only). |
...
Integrates session status into Claude Code's statusline while preserving any statusline you already have. Manage it with `ironbee statusline enable` / `disable`.
...
IronBee runs a local OTEL collector daemon that turns Claude Code's OTLP export into `session_context` (context-usage) events. One daemon per machine, started and reaped automatically. Manage it with [`ironbee otel`](/cli/clients/claude-code#otel-collector).
