...
The four cycles: `browser`, `node`, `backend`, and `android`, share the same set of keys (replace `<cycle>` with `browser`, `node`, `backend`, or `android`):

| Key | Type | Description |
|-----|------|-------------|
| `<cycle>.enable` | `boolean` | Explicit on/off. Written `false` by `ironbee <cycle> disable`, stripped by `ironbee <cycle> enable`. Browser is the only cycle on by default; node, backend, and android are opt-in. |
...
<Note>
  Prefer the [`ironbee browser` / `node` / `backend` / `android` commands](/cli/guides/verification#enable-or-disable-a-platform) for turning cycles on and off; they write `<cycle>.enable` and re-render artifacts for you. Reach for `verifyPatterns` only when you need to change *which files* a cycle covers.
</Note>
...
Each cycle's MCP server can be customized independently via `browserDevTools`, `nodeDevTools`, `backendDevTools`, and `androidDevTools`:
