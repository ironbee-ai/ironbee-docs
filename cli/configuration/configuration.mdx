...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `console.url` | `string` | `https://console.ironbee.ai` | Console base URL, used to build the `🔗 View in IronBee` timeline deep-links in [issue-tracker reports](/cli/guides/issue-tracking#using-it-ironbee-issue-track). `ironbee login` persists whichever console you authenticated against, so self-hosted consoles link correctly. Read live. |
