...
IronBee works out of the box — most projects never need to touch config. When you do want to fine-tune *which* files get verified, change retry limits, or point at a self-hosted collector, this page is the reference.

You can edit settings two ways: the `ironbee config` command (recommended — it validates and re-applies changes for you) or by editing the JSON files directly.
