...
When you run `ironbee install`, IronBee registers a **completion hook** with your AI client (the `Stop` hook in Claude Code, the `stop` hook in Cursor). It fires when the agent tries to finish a task.
...
If verification is [disabled](/cli/guides/verification#enforcement-vs-monitoring-only) (`ironbee verification disable`), the gate is never installed. The agent works unblocked, but session lifecycle, tool calls, and timing still flow to the Collector — you get the observability without the enforcement. Useful for measuring baseline behavior before turning enforcement on.
