...
| `.claude/agents/ironbee-verifier.md` | The **verifier sub-agent** that actually runs the cycles. The devtools MCP servers (one per enabled cycle: `browser-devtools`, `node-devtools`, `backend-devtools`) are rendered **inline** into this file so they're scoped to the sub-agent |
