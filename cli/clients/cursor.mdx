...
| `.cursor/mcp.json` | The devtools MCP servers — one entry per enabled cycle (`browser-devtools`, `node-devtools`, `backend-devtools`, `android-devtools`, `terminal-devtools`) |
...
2. Go to **Settings → Tools & MCP** and confirm the enabled servers (`browser-devtools`, and `node-devtools` / `backend-devtools` / `android-devtools` / `terminal-devtools` if you opted into those cycles) are listed and on.
