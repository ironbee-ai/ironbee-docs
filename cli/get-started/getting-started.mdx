...
Then it walks you through two quick arrow-key pickers: **which verification mode** to run — `enforce` (the default: changes must pass before a task completes), `assist` (tools installed but nothing enforced), or `monitor` (analytics only) — and, unless you chose `monitor`, **which platforms to verify** (browser is on by default; add Node, backend, or Android if you want those gated too). You can change both anytime later (see [Verification](/cli/guides/verification)).
