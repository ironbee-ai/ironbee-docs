...
IronBee detects your AI client (Claude Code, Cursor, or Codex) and sets it up automatically. If it can't detect one, it asks which client you use. It then asks (with an arrow-key picker) **which platforms to verify** — browser is on by default; add Node or backend if you want those gated too. You can change this anytime later (see [Verification](/cli/guides/verification)).
