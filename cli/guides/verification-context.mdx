...
Different parts of a codebase need different things checked. A payments module wants the 3DS challenge exercised; an auth module wants session-fixation verified; the frontend wants an accessibility pass. **Verification context** lets your team author that guidance once, in the repo, and have IronBee surface *only the parts relevant to what changed* — injected straight into the agent's context the moment it starts verifying.
...
- **No match → nothing injected** — areas with no file add no noise.
...
- Your [`ignoredVerifyPatterns`](/cli/configuration/configuration#core-options) filter the set, so test/build/docs churn doesn't pull in guidance.
- IronBee's own `.ironbee/` tree is always excluded.
- `commitDepth` controls how many recent commits join the working tree — `1` (default) covers in-flight work plus the latest commit; `0` is uncommitted-only; raise it for teams that commit per logical step.
...
|------|----------|
| **Enforce** *(default)* | Yes |
| **Assist** | Yes — manual `/ironbee-verify` cycles get the guidance too |
