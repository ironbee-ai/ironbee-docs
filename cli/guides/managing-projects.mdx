...
IronBee detects your AI client (Claude Code or Cursor) and wires up everything it needs: the completion hook, the verification skill and rule, the devtools MCP servers, and the matching permissions. If it can't detect a client, it asks which one you use.
...
ironbee install --client all      # both
```
...
| Option | Description |
|--------|-------------|
| `--client <name>` | Only remove a specific client's setup (`claude`, `cursor`, or `all`). |
