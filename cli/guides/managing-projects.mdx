...
When a selected client can run a headless prompt — **Claude Code** or **Codex**, not a Cursor-only install — the picker also offers an **`s` "suggest"** key. Press it to have IronBee analyze your project with that client's model and **replace** the current selection with a recommended set of cycles; you can still adjust it before pressing Enter. It's opt-in (nothing runs until you press `s`), and if the analysis is cancelled (Esc), times out, or fails, your current selection is kept.

<img src="/images/cli/platform-selection_suggest.png" alt="Platform multi-select with the 's suggest (claude)' key highlighted in the help bar and 'Analysing project with claude…' shown while IronBee recommends which cycles to enable" style={{borderRadius:'8px',width:'100%'}} />

Skip the prompt with `--platforms` (a comma-separated subset of `browser,node,backend,android,terminal`):
...
The selection is written to the committed project config before the artifacts render, so it's equivalent to running the [`ironbee browser` / `node` / `backend` / `android` / `terminal`](/cli/guides/verification#enable-or-disable-a-platform) toggles. Install records the **full** platform state explicitly — an `enable` flag for every cycle (`browser`, `node`, `backend`, `android`, `terminal`) — so the committed `config.json` plainly shows what's on and what's off. In a non-interactive shell with no `--platforms` flag (or in monitoring-only mode, including a just-chosen `--mode monitor`), the platform config is left untouched.
