...
On **Claude Code** or **Codex** the picker also offers an **`s` "suggest"** key. Press it and IronBee analyzes your project with that client's model:
...
<Note>
  Suggestion is opt-in — nothing runs until you press `s` — and needs a headless-capable client (Claude Code or Codex); a Cursor-only install doesn't show the key. If the analysis is cancelled (Esc), times out, or fails, your current selection is kept.
</Note>
...
then hands you a per-check approval list, each check tagged **required** or **advisory** (from its `kind` — typecheck and test default to required):
...
| Flag | Description |
|------|-------------|
| `--client <name>` | Which client's model to analyze with (defaults to a detected Claude / Codex). Cursor has no headless mode, so it can't be used here. |
...
<Note>
  Resolving the suggestion needs a headless-capable client — **Claude Code** or **Codex**. On a Cursor-only project the command has no model to run, so use it from a machine that also has Claude or Codex, or write `verification.checks` by hand.
</Note>
