...
The browser cycle covers most frontend work out of the box. The Node, backend, and Android cycles are off until you opt in, so those changes aren't gated until you ask for it. At install time each platform is a row in the multi-select; space toggles it on.
...
**Android** — device/emulator drive, taps, swipes, screenshots, UI snapshots, Logcat:
