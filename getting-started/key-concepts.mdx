...
Verification is fully enforced. When the agent tries to complete a task, the completion gate triggers automatically and the agent cannot finish until all active verification cycles pass. This is the default after `ironbee install`.

### Assist mode

Verification is enabled the devtools, verification skill, and MCP servers are all available but the completion gate does not automatically block the agent. The agent can run verifications and the results are recorded, but it is not required to pass before completing. Useful when you want the tooling available without strict enforcement.
