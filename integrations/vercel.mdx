...
The check's link opens the session in the Console, where the timeline, the replay, the network payloads, the spans, and the files a finding came from all live. In the Console's verification list these runs carry the **Vercel** trigger, so you can filter for them.
